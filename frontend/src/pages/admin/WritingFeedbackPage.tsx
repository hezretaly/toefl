// src/pages/admin/WritingFeedbackPage.tsx
import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { FileEdit, ArrowLeft, Save, Star, MessageCircle, Loader2, AlertCircle } from 'lucide-react';
  ...
  useEffect(() => {
    if (isAuthenticated && token && responseId) {
      const loadDetails = async () => {
        ...
        try {
          const data = await fetchFeedbackTargetDetails('writing', Number(responseId), token);
          if(data.responseType !== 'writing'){
          ...
          console.error("Error fetching writing feedback details:", err);
  ...
  const handleSubmit = async () => {
      ...
      try {
          ...
          navigate(`/admin/review`);
           ...
           console.error("Error submitting writing feedback:", err);
        ...
        <div className="toefl-container max-w-4xl">
          ...
          <Card className="shadow-lg">
            <CardHeader className='border-b'>
              ...
              <CardTitle className="text-2xl flex items-center gap-2"> <FileEdit className='h-6 w-6 text-toefl-orange'/> Provide Writing Feedback </CardTitle>
            ...
            <CardContent className="p-6 space-y-6">
               ...
               {/* --- FIX: Display Student Response Text --- */}
               <div>
                 <h3 className="font-semibold mb-1 text-gray-800">Student's Response:</h3>
                 <div className='border bg-white p-3 rounded space-y-1 shadow-sm'>
                    {details?.responseText !== null && details?.responseText !== undefined ? (
                        <>
                         <div className="max-h-72 overflow-y-auto text-sm whitespace-pre-wrap">
                            {details.responseText}
                         </div>
                         <p className='text-xs text-gray-500 text-right pt-1 border-t'>Word Count: {details.wordCount ?? 'N/A'}</p>
                        </>
                    ) : (
                         <p className="italic text-gray-500 p-4 text-center bg-gray-100 border rounded">No response submitted.</p>
                    )}
                 </div>
               </div>
               {/* --- END FIX --- */}
               ...
               <div className="pt-6 border-t space-y-6">
                 ...
                 <div>
                   <div className="flex justify-between items-center mb-2"> <label className="font-medium text-gray-700 flex items-center gap-1"> <Star className='h-4 w-4 text-yellow-500'/> Overall Score (0.0 - {SCORE_MAX.toFixed(1)}) </label> <span className="font-bold text-lg text-blue-600">{score.toFixed(1)}</span> </div>
                 ...
                 <div>
                     ...
                     <Textarea id="feedbackText" placeholder="Provide constructive feedback..." className="min-h-[180px]" value={feedbackText} onChange={(e) => setFeedbackText(e.target.value)} />
                 </div>
               </div>
