// src/pages/review/ListeningReviewDetailPage.tsx
import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { Headphones, ArrowLeft, MessageCircle, Star, Loader2, AlertCircle, Check, X, HelpCircle, PlayCircle } from 'lucide-react'; // Added PlayCircle
...
const ListeningReviewDetailPage = () => {
    ...
    useEffect(() => {
        if (isAuthenticated && token && sectionId) {
            const loadDetails = async () => {
                ...
                setError(null);
                try {
                    // Fetch LISTENING details
                    const data = await fetchUserSectionReviewDetails('listening', Number(sectionId), token);
                    setDetails(data);
                } catch (err: any) {
                    console.error("Error fetching listening review details:", err);
    ...
    // Find the main audio for the section/first task if available (structure might vary)
    // This assumes the main listening audio might be linked to the first task/question conceptually
    const mainAudioUrl = details.tasks.length > 0 ? details.tasks[0].taskAudioUrl : null;
        ...
        <div className="flex flex-col min-h-screen">
            ...
            <main className="flex-1 py-8 bg-gray-50">
                <div className="toefl-container">
                    ...
                    <h1 className="text-3xl font-bold mb-6">{details.sectionTitle} - Listening Review</h1>

                     {/* Display Main Listening Audio Once at the top */}
                     {mainAudioUrl && (
                         <Card className="mb-6 shadow">
                            <CardHeader className='p-4 border-b'>
                                <CardTitle className='text-xl font-semibold flex items-center gap-2'><PlayCircle className='h-5 w-5 text-toefl-purple'/> Main Listening Passage</CardTitle>
                            </CardHeader>
                             <CardContent className='p-4 flex justify-center'>
                                 <AudioPlayer src={resolveStaticUrl(mainAudioUrl)} allowReplay={true} />
                             </CardContent>
                         </Card>
                     )}
                                ...
                                <AccordionContent className="pt-2 pb-4 px-4">
                                    <div className="space-y-3">
                                        <div>
                                            ...
                                            {/* Optional: Play snippet audio if available */}
                                            {task.taskAudioUrl && task.taskAudioUrl !== mainAudioUrl && (
                                                <div className='mt-2'>
...
export default ListeningReviewDetailPage;